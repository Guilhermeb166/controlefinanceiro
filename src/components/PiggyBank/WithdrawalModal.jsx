/**
 * Modal para retirada de dinheiro do cofrinho.
 */
"use client"
...
export default function WithdrawalModal({
    ...
    const handleSubmit = () => {
        if (!valor) {
            setError("Digite um valor para retirar")
            return
        }
        ...
        if (numValue > availableBalance) {
            setError(`Saldo insuficiente. Disponível: ${formatCurrency(availableBalance)}`)
            return
        }
    ...
    return (
        ...
        >
            ...
            <Box sx={{ mb: 3 }}>
                <TextField
                    fullWidth
                    label="Motivo da retirada (opcional)"
                    multiline
                    rows={3}
                    value={descricao}
                    onChange={(e) => setDescricao(e.target.value)}
                    placeholder="Ex: Emergência, compra importante..."
                    size="small"
                />
            </Box>
